...
def _resolve_value(value, check_lists=True, check_types=True):
    ...
    if (
        not check_types and not check_lists or not isinstance(value, (list, dict))
    ):  # 3.0 for original, 6.9 total for both, this is 4.8
        return value
    try:
        if check_lists and "append" in value.__class__.__dict__:
    ...
    for key in combined_dict:
        ...
        if "fromkeys" in combined_dict[key].__class__.__dict__:  # 4.2
            ...
            self.__dict__[key] = DotWiz(
                combined_dict[key], _check_lists
